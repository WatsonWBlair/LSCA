...
These datasets are used for benchmarking CAMELS against standardised emotion/sentiment tasks via [MERBench (MERTools)](https://github.com/zeroQiaoba/MERTools).

### MERBench Integration

MERBench accepts pre-extracted per-segment feature arrays (numpy), organised by split and keyed by segment ID.

**Install MERTools:**
```bash
git clone https://github.com/zeroQiaoba/MERTools datasets/merbench
cd datasets/merbench && pip install -r requirements.txt
```

**Extract CAMELS features for MERBench:**
```bash
invoke extract-merbench-features --merbench-root datasets/merbench --datasets "iemocap cmu_mosei meld"
```

Features are written to `datasets/merbench/features/camels/{dataset}/{split}.pkl` as `{segment_id: np.ndarray}` dicts.

**Feature modes** (set via `--feature-mode` on `scripts/extract_merbench_features.py`):
- `concat_768` (default) — concatenates video(768) + phoneme_mean(768) + prosody(22) → (1558,) vector
- `separate` — saves three arrays per segment under keys `"video"`, `"phoneme"`, `"prosody"`

> **Note:** MME-Emotion is skipped — it requires MLLM text generation + GPT-4o judge and is incompatible with frozen-embedding classifiers.

---

### CMU-MOSEI

Multimodal sentiment and emotion dataset with per-utterance labels: sentiment score (−3 to +3) and six binary emotion flags (happy, sad, angry, fearful, disgusted, surprised).

**Access:** HuggingFace or CMU-MultimodalSDK. Expect some video unavailability due to YouTube takedowns.

**Wrangle available segments:**
```bash
invoke wrangle-cmu-mosei --raw-root datasets/cmu_mosei --max-segments 10
```

Output: `datasets/wrangled/MS{batch}/{segment_id}.{mp4,wav,json}`

JSON schema includes `metadata:labels`:
```json
{
  "split": "train",
  "sentiment": 1.2,
  "emotion_flags": {"happy": true, "sad": false, ...}
}
```

Skipped (video unavailable) vs. processed segment counts are logged at completion.

---

### IEMOCAP

Interactive Emotional Dyadic Motion Capture database — 12 hours of scripted and improvised dyadic interactions across 5 sessions. Deferred pending access approval from USC SAIL.

**Access:** Request at https://sail.usc.edu/iemocap/
Once access arrives, MERBench includes pre-extracted features for immediate eval.

---

### MELD

Multimodal EmotionLines Dataset — 1,400 dialogues from Friends TV series with 7 emotion classes. Available directly via GitHub. Consider adding after CMU-MOSEI is stable.
...
The placeholder tables below are auto-populated by that command — do not edit them manually.
